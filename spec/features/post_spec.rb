...
#creates a user local variable available to all below tests
  ...
  let(:post) do
    Post.create(date: Date.today, work_performed: "work_performed", user_id: user.id, daily_hours: 12.5)
  end
...
##----------Index
  describe 'index' do
    ...
    it '- has a scope so that only post creators can see thier posts' do
      ...
      post_from_other_user = Post.create(date: Date.today, work_performed: "addf", user_id: other_user.id, daily_hours: 3.5)
